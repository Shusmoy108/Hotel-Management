...
class RegisterForm(forms.ModelForm):
    pass_conf = forms.CharField(max_length=50, label="Repeat password", widget=forms.PasswordInput)
    isHotelier = forms.BooleanField(label="Register as hotelier", required=False)
...
class ChangePassword(forms.Form):
    ...
    last_pass = forms.CharField(max_length=200, widget=forms.PasswordInput, label='Previous password')
    new_pass = forms.CharField(max_length=200, widget=forms.PasswordInput, label='New Password')
    pass_conf = forms.CharField(max_length=200, widget=forms.PasswordInput, label= 'Verify Password')
    ...
    def clean(self):
        ...
        if not self.user.check_password(last_pass):
            raise forms.ValidationError("Previous password is wrong.")
        else:
